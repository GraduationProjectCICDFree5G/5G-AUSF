pipeline {
    ...
    stages {
        ...
        stage('docker build') {
            steps {
                sh(script: """
                    docker images -a
                    docker build -t 5ggraduationproject/5g-ausf:latest . 
                    docker images -a
                """)
            }
        }
        ...
        stage('Scan Image for Common Vulnerabilities and Exposures') {
            steps {
                sh 'trivy image 5ggraduationproject/5g-ausf --output trivy-report.json'
            }
        }
        stage('Pushing to Dockerhub') {
            steps {
                sh 'docker push 5ggraduationproject/5g-ausf:latest'
            }
        }
    ...
    post {
        always {
            ...
            sh 'docker rmi 5ggraduationproject/5g-ausf'
            sh 'docker rmi 5ggraduationproject/ausf-base'
        }
    }
}
